...
const {
  ...
  PRODUCTION_SFTP_USER,
  PRODUCTION_SFTP_PASSWORD,
  PRODUCTION_PATH
} = require('./load-env.js');

async function deployTheme() {
  console.log('🚀 Deploying custom theme to PRODUCTION...\n');
  console.log(`Target: ${PRODUCTION_PATH}/wp-content/themes/client-website\n`);
  ...
  return new Promise((resolve, reject) => {
    conn.on('ready', () => {
      ...
      conn.sftp((err, sftp) => {
        ...
        const themePath = `${PRODUCTION_PATH}/wp-content/themes/client-website`;
    ...
    conn.connect({
      host: PRODUCTION_SFTP_HOST,
      port: parseInt(PRODUCTION_SFTP_PORT),
      username: PRODUCTION_SFTP_USER,
      password: PRODUCTION_SFTP_PASSWORD
    });
  });
}
